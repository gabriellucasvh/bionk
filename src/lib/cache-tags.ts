...
export function profilePageCacheKey(username: string): string {
	return `cache:profile:page:${username}`;
}

export async function evictProfilePageCache(username: string): Promise<void> {
	const r = getRedis();
	await r.del(profilePageCacheKey(username));
}
