...
import { ensureHttps, fetchMetadataFromProvider, parseMusicUrl, resolveDeezerShortUrl } from "@/utils/music";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!parsed.id || parsed.type === "unknown" || parsed.platform === "unknown") {
      return NextResponse.json({ error: "URL de música inválida" }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Falha ao obter metadados" }, { status: 502 });
    }
  } catch (error) {
