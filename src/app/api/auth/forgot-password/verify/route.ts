...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const redis = getRedis();
		const attemptsRaw = await redis.get<number | null>(attemptsKey);
		...
		const storedHash = (await redis.get<string | null>(hashedKey)) as
			| string
			| null;
		if (!storedHash) {
		...
		const plainToken = (await redis.get<string | null>(tokenPlainKey)) as
			| string
			| null;
		if (!plainToken) {
			return NextResponse.json(
