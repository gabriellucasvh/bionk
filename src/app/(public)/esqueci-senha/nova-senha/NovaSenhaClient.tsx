...
export default function NovaSenhaClient({
	...
	const dict = require(
		`@/dictionaries/public/${normalizeLocale(locale)}/nova-senha.ts`
	).default;
	...
	async function handleReset() {
		...
		try {
			const res = await fetch("/api/auth/forgot-password/reset", {
				...
				body: JSON.stringify(
					token ? { token, password: pwd } : { password: pwd }
				),
			});
	...
	return (
		<div className="mx-auto flex h-dvh w-full max-w-lg flex-col items-center justify-center px-4 text-center">
			...
			<div className="mt-6 w-full space-y-2">
				<Label htmlFor="pwd">{dict.newPassword}</Label>
			...
			<div className="mt-4 w-full space-y-2">
				...
				<div className="relative">
					...
					<button
						aria-label={
							showConfirm ? "Ocultar confirmação" : "Exibir confirmação"
						}
						className="-translate-y-1/2 absolute top-1/2 right-2"
						onClick={() => {
			...
			<div className="mt-6 flex w-full gap-3">
				<BaseButton
					...
					onClick={handleReset}
				>
					{loading ? dict.resetting : dict.saveNew}
				</BaseButton>
			</div>
		</div>
	);
