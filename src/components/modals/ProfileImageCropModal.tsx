...
// Lista de formatos de imagem aceitos
...
const CROP_BYPASS_FORMATS = [
	"image/gif", // GIFs devem pular o crop para preservar a animação
	"image/svg+xml",
	"image/heic",
	"image/heif",
];
...
const ProfileImageCropModal: FC<ProfileImageCropModalProps> = ({
	isOpen,
	onClose,
	onImageSave,
	currentImageUrl,
	...
	const handleFileSelect = (file: File) => {
		...
		// Validar tamanho do arquivo (máximo 2MB para perfil)
	...
	const processImage = useCallback(async () => {
		...
		try {
			...
			if (CROP_BYPASS_FORMATS.includes(selectedImage.file.type)) {
				...
				imageFile = new File([imageBlob], selectedImage.file.name, {
					type: "image/png",
				});
			}
	...
	}, [selectedImage, croppedAreaPixels, onImageSave, handleClose]);
	...
	}, [selectedImage, shouldShowCropper, isProcessing, processImage]);
			...
			<DialogContent className="w-full max-w-[90vw] rounded-2xl border bg-background p-6 shadow-xl sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription className="text-center text-gray-600 dark:text-white/80">
						Ajuste a imagem para criar a foto de perfil perfeita.
					</DialogDescription>
				</DialogHeader>
					...
					{shouldShowCropper ? (
						...
						<div className="space-y-4">
							...
							<div
								...
								onClick={() =>
									document.getElementById("profile-file-upload-input")?.click()
								}
